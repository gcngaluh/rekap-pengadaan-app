...
# ==========================================
# 2. KONFIGURASI DATABASE GOOGLE SHEETS
# ==========================================
...
def get_data():
    """Mengambil data dari Google Sheets dan memastikan formatnya sah."""
    try:
        # ttl=0 memastikan data selalu ditarik baru dari server, menghindari cache lama
        ...
        # Penyesuaian tipe data standar
        df['id'] = pd.to_numeric(df['id'], errors='coerce').fillna(0).astype(int)
        df['tahun_anggaran'] = pd.to_numeric(df['tahun_anggaran'], errors='coerce').fillna(2026).astype(int)
        # Konversi string punya_npwp dari GSheets ke Boolean
        df['punya_npwp'] = df['punya_npwp'].astype(str).str.upper().isin(['TRUE', '1', 'YA'])
        
        # Konversi kolom numerik keuangan
...
def hitung_pajak(bruto, kategori_pajak, punya_npwp, manual_rate=0.0):
    ...
    if "Barang" in kategori_pajak:
        kat_engine, pph_rate = "Barang", 0.015 if punya_npwp else 0.030
    elif "Jasa" in kategori_pajak:
        kat_engine, pph_rate = "Jasa", 0.020 if punya_npwp else 0.040
    elif "Sewa" in kategori_pajak:
        kat_engine, pph_rate = "Sewa", 0.100
    elif "1.75%" in kategori_pajak:
        kat_engine, pph_rate = "Konstruksi", 0.0175
    elif "2.65%" in kategori_pajak:
        kat_engine, pph_rate = "Konstruksi", 0.0265
    elif "3.50%" in kategori_pajak:
        kat_engine, pph_rate = "Konstruksi", 0.0350
    elif "4.00%" in kategori_pajak:
        kat_engine, pph_rate = "Konstruksi", 0.0400
    elif "6.00%" in kategori_pajak:
        kat_engine, pph_rate = "Konstruksi", 0.0600
    elif "Bebas Pajak" in kategori_pajak:
        return bruto, 0, 0, bruto
    elif "Manual" in kategori_pajak:
        kat_engine, pph_rate = "Lainnya", manual_rate / 100.0
...
def main_dashboard():
    st.markdown("""
        ...
        div[data-testid="stTooltipContent"] { width: max-content !important; max-width: 350px !important; white-space: normal !important; z-index: 99999 !important; }
        .stButton > button { padding: 4px 8px !important; height: auto !important; min-height: 32px !important; }
        </style>
    ...
    df_utama = get_data() # Muat data dari Google Sheets

    c_title, c_logout = st.columns([4, 1])
    with c_title:
        st.title(f"📊 Dashboard Rekapitulasi (TA {st.session_state.tahun_anggaran})")
        st.write("Pencatatan nilai kuitansi, PPN, dan PPh tersinkronisasi dengan Google Sheets.")
    with c_logout:
    ...
    if st.button("Hitung & Simpan ke Database", use_container_width=True, type="primary"):
        ...
        if not nama_pengadaan: st.error("Nama pengadaan tidak boleh kosong!")
        elif nilai_bruto <= 0: st.error("Nilai pengadaan harus lebih dari Rp 0!")
        else:
            ...
            data_baru = pd.DataFrame([{
                ...
                "nama_pengadaan": nama_pengadaan, "jenis_pajak": pajak_tersimpan, "punya_npwp": punya_npwp,
    ...
    # Filter data hanya untuk tahun anggaran aktif
    ...
    if not df_filter.empty:
        ...
        for _, row in df_filter.iterrows():
            if st.session_state.edit_id == row['id']:
                    ...
                    with st.form(key=f"form_edit_{row['id']}"):
                        ...
                        if col_btn1.form_submit_button("💾 Simpan", type="primary"):
                            ...
                            if not edit_nama or val_bruto <= 0:
                                ...
                                df_utama.at[idx_utama, 'punya_npwp'] = edit_npwp
                ...
                c5.write("✅" if row['punya_npwp'] else "❌")
                ...
                with c12:
                    if st.session_state.confirm_del_id == row['id']:
                        ...
                        if col_aksi1.button("✏️", key=f"btn_edit_{row['id']}", use_container_width=True):
                            st.session_state.edit_id = row['id']
                            st.session_state.confirm_del_id = None
                            st.rerun()
                        if col_aksi2.button("🗑️", key=f"btn_del_{row['id']}", use_container_width=True):
